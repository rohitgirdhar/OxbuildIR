...
void runSearch(string dir,
    string query,
    const vector<float>& bounding_box,
  ...
  map<int, int> vws;
  readDescriptorsWithCounts(dir + "/" +
      query + ".txt", bounding_box, vws);
...
int main(int argc, char *argv[]) {
  ...
  if (query.compare("-1") == 0) {
    ...
    for (int test_i = 0; test_i < img_names.size(); test_i++) {
      vector<tuple<string, float, int>> ids;
      runSearch(vm["dir"].as<string>(),
          img_names[test_i],
          bounding_boxes[test_i],
          invIdx,
          imgStats,
          ids,
          vm["geom-rerank"].as<bool>());
      ofstream fout((output_dir + "/" + img_names[test_i] + ".out").c_str(), ios::out);
      ...
      fout.close();
      cerr << "Done for " << img_names[test_i] << endl;
    }
    fin.close();
  } else {
